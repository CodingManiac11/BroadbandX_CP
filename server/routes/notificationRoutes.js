...
router.get('/reminders', authMiddleware, async (req, res) => {
  try {
    ...
    for (const subscription of subscriptions) {
      const nextBillingDate = new Date(subscription.nextBillingDate || subscription.endDate);
      ...
      if (daysUntilDue <= 7 && daysUntilDue > 0) {
        reminders.push({
          ...
          message: `Your ${subscription.plan.name} subscription payment of ₹${subscription.pricing.totalAmount} is due in ${daysUntilDue} days`,
        ...
        reminders.push({
          ...
          message: `Your ${subscription.plan.name} subscription payment is overdue by ${Math.abs(daysUntilDue)} days`,
          dueDate: nextBillingDate,
          amount: subscription.pricing.totalAmount,
          priority: 'urgent',
          read: false,
